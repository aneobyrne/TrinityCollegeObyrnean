...
int bstdb_add ( char *name, int word_count, char *author ) { //creates new node in the BST, with the book name, word count and author
 int doc_id = next_id++; //next ID is an incremented unique ID
...
void bstdb_stat ( void ) { //prints out statistics of the tree
    ...
    float avg_traversal = calculate_avg_traversal(root); //3. Calculate average node traversal
    printf("Average traversal of nodes: %.2f\n", avg_traversal);

    int unique_ids = check_unique_ids(root, NULL, 0); //4. Check all ID's to prove uniqueness
      if (unique_ids) 
...
BookNode *rotateRight(BookNode *root) { //rotate right
    BookNode* new_root = root->left; //left subtree root is new root
    root->left = new_root->right; //righ child of the new root is now left child of original root
...
BookNode* left_rotate(BookNode* root) { //rotate left
    ...
    root->right = new_root->left; //left child of new root is now right child of original root
...
int check_balance(BookNode *node) { //function to check if tree is balanced
    ...
    if (abs(left_height - right_height) > 1) //calculates the absolute paths difference, if too big its unbalanced
      return 0;
    return check_balance(node->left) && check_balance(node->right); //continue checking not already unbalanced
}

int calculate_height(BookNode *node) { //longest path from root to a leaf node (number of its levels)
    if (!node) //if NULL, level is 0
     return 0;
    int left_height = calculate_height(node->left); //continue to left nodes
    int right_height = calculate_height(node->right); //conmtinue to right nodes
     return 1 + (left_height > right_height ? left_height : right_height); //returns longest path plus the current 1
}
...
int check_unique_ids(BookNode *node, int *visited_ids, int index) { //checks IDs in the document
    ...
    return check_unique_ids(node->left, visited_ids, index + 1) && //check next tree nodes
           check_unique_ids(node->right, visited_ids, index + 1);
}
